...
void print_elf_header32(Elf32_Ehdr ehdr){
	...
	printf("Entry point address : 0x%06u\n",ehdr.e_entry);
...
void print_sec_header32(int fd, Elf32_Ehdr ehdr, Elf32_Shdr shdrs[]){
	...
	for(i=0;i<ehdr.e_shnum;i++){
		...
		printf("%-16s ",(string_table+shdrs[i].sh_name));
		printf("0x%016X ",shdrs[i].sh_type);
		printf("0x%016u ",shdrs[i].sh_addr);
		printf("0x%08u ",shdrs[i].sh_offset);
		
		printf("\n      ");

		printf("0x%016u ",shdrs[i].sh_size);
		printf("0x%016u ",shdrs[i].sh_entsize);
	
		remain_space = 7;
		flag = (int)shdrs[i].sh_flags;

		printf("%7u ",shdrs[i].sh_flags);
		printf("%-6X ",shdrs[i].sh_link);
		printf("%-6X ",shdrs[i].sh_info);
		printf("%-6u ",shdrs[i].sh_addralign);
...
void print_pro_header32(Elf32_Ehdr ehdr, Elf32_Phdr *phdrs){
	...
	for(i=0;i<ehdr.e_phnum;i++){
		...
		printf("0x%016u ",phdrs[i].p_offset);
		printf("0x%016u ",phdrs[i].p_vaddr);
		printf("0x%016u ",phdrs[i].p_paddr);
		printf("\n                ");
		printf("0x%016u ",phdrs[i].p_filesz);
		printf("0x%016u ",phdrs[i].p_memsz);
		printf("%-8X ",phdrs[i].p_flags);
		printf("%7u ",phdrs[i].p_align);
		printf("\n");
	}
}
